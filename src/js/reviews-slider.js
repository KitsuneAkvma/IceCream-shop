...
const icons = Array.from(document.querySelectorAll('.reviews__slide__nav__indicator .icon'));
const person = document.querySelector('.reviews__slide__nav__person');

const comments = Array.from(document.querySelectorAll('.reviews__slide__track > *'));
...
function showSlide(commentIndex) {
  ...
  icons.forEach((icon, index) => {
    icon.style.fill = index === commentIndex ? slideData[commentIndex].color : '#d9d9d9';
  });
